...
def CreateCircularMesh(path, mesh_name, R, h, plot_info):
    ...
    fmesh, mf = gmsh2dolfin(path, mesh_name, '2D', bord_string_tag, surface_string_tag)            

    return fmesh, mf, tag_bords

def CreateRectangularMesh(path, mesh_name, Lx,Ly, h, plot_info):
    # Create membrane mesh with Gmsh
    gmsh.initialize()
    gmsh.option.setNumber("General.Terminal", 1)
    gmsh.model.add(mesh_name)
