...
def boundary_grid_from_fenics_mesh(fenics_mesh, dimension,fenics_space):
    """
    ...
    if dimension=='3D':
        boundary_mesh = _dolfin.BoundaryMesh(fenics_mesh, "local", False)
        ...
        boundary_mesh = fenics_mesh                 # triangle_mesh
        bm_coords = boundary_mesh.coordinates()     # triangle_mesh.points
        bm_cells = boundary_mesh.cells()            # triangle_mesh.cells[0][1]
        bm_nodes = np.uint64(fenics_space.dofmap().dofs())
        bempp_boundary_grid = bempp.api.Grid(bm_coords.transpose(), bm_cells.transpose())
    
    return bempp_boundary_grid, bm_nodes


def fenics_to_bempp_trace_data(fenics_space, dimension):
    """Return tuple (space,trace_matrix)."""
    family, degree = fenics_space_info(fenics_space)
    
    if family == "Lagrange":
        if degree == 1:
            return p1_trace(fenics_space,dimension)
        else:
...
def p1_trace(fenics_space, dimension):
    """
    ...
    bempp_boundary_grid, bm_nodes = boundary_grid_from_fenics_mesh(fenics_mesh, dimension,fenics_space)
